...
class VehicleService {
    ...
    async getAllVehicles() {
        const result = await server_1.pool.query("SELECT * FROM vehicles ORDER BY id DESC");
        return result.rows;
    }
    async getVehicleById(id) {
        const result = await server_1.pool.query("SELECT * FROM vehicles WHERE id=$1", [id]);
        return result.rows[0];
    }
    async updateVehicle(id, data) {
        ...
        const result = await server_1.pool.query(query, [...values, id]);
        return result.rows[0];
    }
    async updateAvailability(id, status) {
        const result = await server_1.pool.query(`UPDATE vehicles SET availability_status=$1 WHERE id=$2 RETURNING *`, [status, id]);
        return result.rows[0];
    }
    async deleteVehicle(id) {
        await server_1.pool.query(`DELETE FROM vehicles WHERE id=$1 CASCADE`, [id]);
